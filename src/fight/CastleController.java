...
public class CastleController{
    ...
    //Animation variables
    //player
    public int warriorStartX = 170, warriorStartY = 250, warriorX = warriorStartX, warriorY = warriorStartY;
    public int rangerStartX = 70, rangerStartY = 310, rangerX = rangerStartX, rangerY = rangerStartY;
    public int mageStartX = -110, mageStartY = 310, mageX = mageStartX, mageY = mageStartY;
    public int healerStartX = -30, healerStartY = 250, healerX = healerStartX, healerY = healerStartY;

    public int undead1X = 750, undead1Y = 300, undead1StartX = undead1X, undead1StartY = undead1Y;
    public int undead2X = 1005, undead2Y = 300, undead2StartX = undead2X, undead2StartY = undead2Y;
    public int undead3X = 870, undead3Y = 350, undead3StartX = undead3X, undead3StartY = undead3Y;
    public int undead4X = 1125, undead4Y = 350, undead4StartX = undead4X, undead4StartY = undead4Y;
    ...
    private void enemyDamage(){
        for (int i = 0; i < undeadHp.length; i++) {
            if (undeadHp[i] > 0) {
                undeadAttack();
                partyDeath();
            }
    ...
    public void targetSystem() {
        ...
        cv.undead1.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                target = 1;
                cv.targetarrow.setLocation(undead1StartX + 25, undead1StartY - 100);
                cv.targetarrow.setVisible(true);
            }
        });
        cv.undead2.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                target = 2;
                cv.targetarrow.setLocation(undead2StartX + 25, undead2StartY - 100);
                cv.targetarrow.setVisible(true);
            }
        });
        cv.undead3.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                target = 3;
                cv.targetarrow.setLocation(undead3StartX + 25, undead3StartY - 100);
                cv.targetarrow.setVisible(true);
            }
        });
        cv.undead4.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                target = 4;
                cv.targetarrow.setLocation(undead4StartX + 25, undead4StartY - 100);
                cv.targetarrow.setVisible(true);
            }
        });
    }
    ...
    private void attackPressed() {
        ...
        if (turns == 1 && warriorEnergyInt > 1 && cv.targetarrow.isVisible()) {
            ...
            tackle.start(); //Warrior deals damage to a undead.
        }
        //If its ranger's turn and player has 2 or more energy.
    ...
    private void isFightOver() {
        ...
        if (undeadHp[0] < 1 && undeadHp[1] < 1 && undeadHp[2] < 1 && undeadHp[3] < 1) {
            MusicPick.musicStop();
            cv.CastleFightJFrame.dispose();
            fightWon = true;
    ...
    public void setStartLabels() {
        ...
        cv.undead1Hp = new JLabel("undead 1: " + undeadHp[0]);
        cv.undead2Hp = new JLabel("undead 2: " + undeadHp[1]);
        cv.undead3Hp = new JLabel("undead 3: " + undeadHp[2]);
        cv.undead4Hp = new JLabel("undead 4: " + undeadHp[3]);
    ...
    //When the undead attacks.
    private void undeadAttack() {
        ...
        while (true) {
            ...
            if (target == 0) {
                ...
                if (warriorCurrentHp > 0) {
                    ...
                    enemyDamage = enemyDamage - warriorBlock; //Warrior take damage equal to undead damage.
    ...
    private void mobDeath() {
        ...
        if (undeadHp[0] <= 0) {
            cv.undead1Hp.setText("undead 1: 0");
            cv.undead1.setVisible(false);
            if (target == 1) {
                cv.targetarrow.setVisible(false);
            }
        }
        if (undeadHp[1] <= 0) {
            cv.undead2Hp.setText("undead 2: 0");
            cv.undead2.setVisible(false);
            if (target == 2) {
                cv.targetarrow.setVisible(false);
            }
        }
        if (undeadHp[2] <= 0) {
            cv.undead3Hp.setText("undead 3: 0");
            cv.undead3.setVisible(false);
            if (target == 3) {
                cv.targetarrow.setVisible(false);
            }
        }
        if (undeadHp[3] <= 0) {
            cv.undead4Hp.setText("undead 4: 0");
            cv.undead4.setVisible(false);
            if (target == 4) {
                cv.targetarrow.setVisible(false);
            }
        }
    ...
    public void spellDamageSystem(int unbuffedDamage, String damageTargets) {
        ...
        cv.undead1Hp.setText("undead 1: " + undeadHp[0]);
        cv.undead2Hp.setText("undead 2: " + undeadHp[1]);
        cv.undead3Hp.setText("undead 3: " + undeadHp[2]);
        cv.undead4Hp.setText("undead 4: " + undeadHp[3]);
        mobDeath();
        isFightOver();
    }
    ...
    public Timer demoralized = new Timer(20, new ActionListener() {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (phase == 0) {
                if (undeadHp[0] > 0) cv.demoSword1.setVisible(true);
                if (undeadHp[2] > 0) cv.demoSword2.setVisible(true);
                if (undeadHp[1] > 0) cv.demoSword3.setVisible(true);
                if (undeadHp[3] > 0) cv.demoSword4.setVisible(true);
                upMegaMath *= 2;
                swordIconY -= upMegaMath;
            }
            if (timePast % 3 == 0) {
    ...
    private Timer enemyTurnTimer = new Timer(7, new ActionListener() {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            cv.targetarrow.setVisible(false);
            if (timePast < 50) {
                if (undeadHp[0] < 1) timePast += 100;
            } else if (timePast < 60) {
                undead1X -= 15;
                cv.undead1.setLocation(undead1X, undead1Y);
            } else if (timePast < 70) {
                undead1X += 15;
                cv.undead1.setLocation(undead1X, undead1Y);
            } else if (timePast < 150) {
                cv.undead1.setLocation(undead1StartX, undead1StartY);
                if (undeadHp[1] < 1) timePast += 100;

            } else if (timePast < 160) {
                undead2X -= 15;
                cv.undead2.setLocation(undead2X, undead2Y);
            } else if (timePast < 170) {
                undead2X += 15;
                cv.undead2.setLocation(undead2X, undead2Y);
            } else if (timePast < 250) {
                cv.undead2.setLocation(undead2StartX, undead2StartY);
                if (undeadHp[2] < 1) timePast += 100;

            } else if (timePast < 260) {
                undead3X -= 15;
                cv.undead3.setLocation(undead3X, undead3Y);
            } else if (timePast < 270) {
                undead3X += 15;
                cv.undead3.setLocation(undead3X, undead3Y);
            } else if (timePast < 350) {
                cv.undead3.setLocation(undead3StartX, undead3StartY);
                if (undeadHp[3] < 1) timePast += 100;

            } else if (timePast < 360) {
                undead4X -= 15;
                cv.undead4.setLocation(undead4X, undead4Y);
            } else if (timePast < 370) {
                undead4X += 15;
                cv.undead4.setLocation(undead4X, undead4Y);
            } else if (timePast == 380) {
                cv.undead4.setLocation(undead4StartX, undead4StartY);
                enemyDamage();
                timePast = 0;
                enemyTurnTimer.stop();
                cv.endTurnButton.setVisible(true);
